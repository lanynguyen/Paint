...
type shape = 
  | Line of {color: color; t: bool; p1: point; p2: point}
  | Points of {color: Gctx.color; t: bool; points: point list}
  | Ellipse of {color: Gctx.color; t: bool; p: point; x: int; y: int}
...
let repaint (g: gctx) : unit =
  let draw_shape (s: shape) : unit =
    begin match s with
      ...
      | Ellipse e -> draw_ellipse (with_params g e.color e.t) e.p e.x e.y
    end in
  ...
  begin match (event_type event) with
    | MouseDown ->
      ...
      (begin match paint.mode with 
          ...
          | PointMode -> paint.preview <- 
            Some (Points {color=paint.color; t = paint.thickness; points = [p]})
    ...
    | MouseDrag ->
      ...
      | LineEndMode p1 -> paint.preview <- 
              Some (Line {color=paint.color; t = paint.thickness; p1=p1; p2=p})
      | PointMode -> 
          let points_list = 
            begin match paint.preview with
              ...
              | _ -> []
                ...
                Some (Points {color=paint.color; t = paint.thickness; 
                points = points_list @ [p]})
      | EllipseStartMode -> paint.preview <- None
      | EllipseEndMode p1 -> 
        let center : point = ((fst p + fst p1) / 2, (snd p + snd p1) / 2) in 
        let rx = (abs (fst p - fst p1)) / 2 in
        let ry = (abs (snd p - snd p1)) / 2 in
        paint.preview <- 
            Some (Ellipse {color=paint.color; t = paint.thickness; p=center; 
            x=rx; y=ry})
      end
    | MouseUp ->
      ...
      | EllipseEndMode p1 ->
          ...
          let ry = (abs (snd p - snd p1)) / 2 in
          Deque.insert_tail
                  (Ellipse {color=paint.color; t = paint.thickness; p=center; 
                  x=rx; y=ry}) paint.shapes;
...
(** Create the Thickness button *)
let (w_thickness, nc_thickness) = checkbox false "Thin line"

;; nc_thickness.add_change_listener (fun b -> paint.thickness <- b)

(** Create color slider widget *)

let (w_blue, nc_blue) = Widget.slider 0 Gctx.blue 

;; nc_blue.add_change_listener 
    (fun x -> paint.color <- {r = paint.color.r; g = paint.color.g; b = x})

let (w_green, nc_green) = Widget.slider 0 Gctx.green

;; nc_green.add_change_listener 
    (fun x -> paint.color <- {r = paint.color.r; g = x; b = paint.color.b})
let (w_red, nc_red) = Widget.slider 0 Gctx.red

;; nc_red.add_change_listener 
    (fun x -> paint.color <- {r = x; g = paint.color.g; b = paint.color.b})
...
let mode_toolbar : widget = hlist [border w_undo; spacer; border w_line; spacer;
                                  border w_point; spacer; border w_ellipse; 
                                  spacer; border w_thickness]
...
(* TODO: Task 1 - This code contains a great deal of boilerplate.  You
   ...
   spacer; color_button magenta; spacer; 
   vlist [fst (label "Color Slider:"); border w_blue; border w_red; 
   border w_green]]
