...
let make_controller (v: 'a) : 'a value_controller =
  let value_controller = ref v in
  let listeners = ref [] in
  { 
  add_change_listener = (fun (x: 'a -> unit) -> listeners.contents <- 
                                                    listeners.contents @ [x]);
  get_value = (fun () -> value_controller.contents);
  change_value = (fun x -> List.iter (fun y -> y x) listeners.contents;
                                              value_controller.contents <- x)
  }                             
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
  let (w_controller, lc_controller, nc_controller) = button s in
  let controller = make_controller init in
  let click () : unit =
    (if controller.get_value ()
    then (controller.change_value false; lc_controller.set_label s)
    else (controller.change_value true; lc_controller.set_label ("Thick line")))
    in
    nc_controller.add_event_listener (mouseclick_listener click);
...
let slider (init: int) (c: Gctx.color) : widget * int value_controller =
  (* Create value controller *)
  let controller = make_controller init in
  (* Create actual widget *)
  let slider_widget =
  {
  (* Draw the changing color rectangle *)
  repaint = (fun g -> Gctx.fill_rect (Gctx.with_color g c) (0, 0) 
                                    (controller.get_value(), 10));
  (* Handle user's mouse actions *)                                  
  handle = (fun (g: Gctx.gctx)(e: Gctx.event) ->
          if event_within g e (255, 10) then
              if Gctx.event_type e = Gctx.MouseDrag || 
                  Gctx.event_type e = Gctx.MouseDown 
              then controller.change_value (fst (Gctx.event_pos e g))
              else()
           else()); 
  (* Size of each slider *)           
  size = (fun () -> (255, 10))
  } in 
  (slider_widget, controller)                                  